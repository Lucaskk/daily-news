...
自 2026-09-11 晚間起，缺件告警包含原因、最後產製階段、判斷依據、公開頁檢查結果及處理方式，不只寫「發布異常」。

- 額度耗盡：讀取當日最新 Codex 執行的 `usageLimitExceeded` 系統錯誤碼；模型不必還能回覆，既有 watchdog 就能判讀。
- 請求頻率限制與用量上限分開，不將一般 HTTP 429 或網路問題直接說成額度耗盡。
- GitHub 推送、Pages 驗證、LINE 配送與研究產製失敗，依當日 production checkpoint 顯示失敗階段；仍執行中則如實標示，不能宣稱已失敗。
- 只唯讀目前新聞任務當日 08:00 之後的最新執行狀態，不讀取其他任務、對話全文、工具內容、憑證或完整歷史。新執行／較新進度會取代舊錯誤，不沿用前日額度告警。
- 通知只使用固定繁體中文分類與時間，不轉傳原始系統錯誤、私人路徑、token 或收件者。未提供可確認的額度恢復時間時不自行猜測。
- 本機執行狀態資料庫是版本相依的可選來源；不存在、格式改變或無法讀取時照常告警，明寫「原因尚未確認」，不阻擋配送檢查。
- 維持 10:00 後每日一次缺件告警及既有重試去重。已送出的訊息不改寫、不為本次介面更新另發假告警；下一次真正缺件時套用新格式。

示例（非當日真實故障）：

> 原因：Codex 模型使用額度已達上限，當次執行無法繼續。
> 進度：研究與日報產製。
> 依據：Codex 當日執行於 08:12:00 回報失敗（台北時間）。
> 處理：額度恢復後需接續產製；恢復時間無法確認時不猜測。

新增 13 項診斷測試，watchdog 共 32 項測試通過，另保留 15 項 pipeline 回歸測試。
正式入口已以原子替換安裝並通過相同 32 項測試；17:51:44 實際執行回報 `already sent`，未重複發送。
